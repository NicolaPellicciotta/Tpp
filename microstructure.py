...
class Stroke():
    def __init__(self,coors,speed,ipstep,holo=None):        
        ...
        #self.scanRecord=None #the recorded coordinates during a piezo scan
        #self.scanStdDev=None   #the standard deviation from the scan coordinates
...
class MicroStr():
    ...
    def plot(self,plotmode=0,markerscalef=0.1):
        ...
        scf=markerscalef
        fh=mlab.figure(fgcolor=(1,1,1),size=(750,750))
...
#        fh.scene.disable_render=True
        ...
        for s in self.Strokes:
            ...
            mlab.points3d(s.coors[:,0],s.coors[:,1],s.coors[:,2],color=markercol,scale_factor=scf,mode='cube')
            mlab.plot3d(s.coors[:,0],s.coors[:,1],s.coors[:,2],color=linecol,tube_radius=None,line_width=2.)                 
            ci+=1           
        ...
        mlab.points3d(clims[:,0],clims[:,1],clims[:,2],color=(0,0,0),scale_factor=0.025)
...
def textStr(txt,textsize,speed):
    ...
    for i in range(ln):
        ...
        fstring+=str(textsize) + "," + str(xpos) + "," + str(ypos) + "," + str(speed) + ")"
        exec fstring
        if i==0: txstr=deepcopy(lstr)
        else: txstr.addStr(lstr)
