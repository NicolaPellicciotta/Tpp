...
class chart(monitor):
  def __init__(self, roi=None, timewindow=60, normed=False):
    super(chart, self).__init__(roi)
    ...
    fig=MonitorPlot(self)
    self.track=MonitorCurve()
    self.track.attach(fig)
    layout.addWidget(fig)
    #fig.setTitle("horizontal slice")
    fig.setAxisTitle(Qwt.QwtPlot.xBottom, "time")
  ...
  def update(self):
    data = self.roi.data.flatten()
    peak=sort(data)[-10:].mean()
    if self.normed:
      peak = peak/data.sum()
    self.data+=[peak]
    ...
    cond=(x[-1]-x)<self.timewindow
    x=x[cond]
    y=y[cond]

    if len(self.data)>1:
      self.track.setData(x, y)
      self.fig.zoomer.setZoomBase(QtCore.QRectF(0,min(y), len(y), max(y)-min(y)))
      self.fig.replot()
