...
class MicroStr():
    ...
    def plot(self,plotmode=0,markerscalef=0.1):
        ...
        scf=markerscalef
        fh=mayalab.figure(fgcolor=(1,1,1),size=(750,750))
...
#        fh.scene.disable_render=True
        ...
        for s in self.Strokes:
            ...
            mayalab.points3d(s.coors[:,0],s.coors[:,1],s.coors[:,2],color=markercol,scale_factor=scf,mode='cube')
            mayalab.plot3d(s.coors[:,0],s.coors[:,1],s.coors[:,2],color=linecol,tube_radius=None,line_width=2.)                 
            ci+=1           

        eng=mayalab.get_engine()  
        ...
        mayalab.axes(nb_labels=5,ranges=clims.T.flatten())
