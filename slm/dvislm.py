...
class DVImodulator:
    ...
    def show(self, holo):
        ...
        #prepare hologram
        if holo.dtype!=uint8: self.displayedholo=rad2byte(holo)
        else: self.displayedholo=holo.copy()

        if self.slm_shift is not None:
            self.displayedholo=roll(self.displayedholo,self.slm_shift[0],axis=0)
            self.displayedholo=roll(self.displayedholo,self.slm_shift[1],axis=1)
        if self.aberration is not None:
            self.displayedholo+=self.aberration
        if self.lut is not None:      
            self.displayedholo=self.lut[self.displayedholo]
        #show hologram
